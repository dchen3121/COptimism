...
def ultimate_check(x, y, board, color):
    if is_in_check(board, color):
        return moves_while_in_check(board, color)
    else:
        return valid_moves(x, y, board)


def is_in_check(board, color):
    king = board.search(Piece(Type.KING, color))
    ...
    for x in range(0, 8):
        for y in range(0, 8):
            piece = board.get(x, y)
            if piece is not None and piece.color == color.other() and piece.type != Type.KING:
                oppositeMoves.extend(valid_moves(x, y, board))
...
def valid_move_pawn(x, y, board, color):
    ...
    if color == Color.WHITE:
        ...
        if y == 1 and board.get(x, y + 2) is None and board.get(x, y + 1) is None:
        ...
        if y == 6 and board.get(x, y - 2) is None and board.get(x, y - 1) is None:
            posMoves.append((x, y - 2))
...
def valid_move_knight(x, y, board, color):
    ...
    while index <= len(counters_list) - 1:
        if in_bound(x + counters_list[index][0], y + counters_list[index][1]) and \
            ...
            moves_knight.append((x + counters_list[index][0], y + counters_list[index][1]))
        index += 1
